function [model1,model2,model3,model4] = SVM_training(method, sift_type, vocabulary_size, kernel)
%used to train SVM classifier
    ...
    model1 = fitcsvm(histograms_all, label1,'KernelFunction',kernel,'ClassNames',[0, 1]); %linear
    model2 = fitcsvm(histograms_all, label2,'KernelFunction',kernel,'ClassNames',[0, 1]); %linear  
    model3 = fitcsvm(histograms_all, label3,'KernelFunction',kernel,'ClassNames',[0, 1]); %linear
    model4 = fitcsvm(histograms_all, label4,'KernelFunction',kernel,'ClassNames',[0, 1]); %linear
